...
public class YoloApi {
    ...
    public static int errorOrderNumLine1;
    public static int errorOrderNumLine2;
    public static int errorOrderNumLine3;

    @Autowired
    private OrderService orderService;

    @Autowired
    public YoloApi(ResourceLoader resourceLoader) {
    ...
    public Map<String,Object> upload(@RequestParam("file") MultipartFile file){
        ...
        if (detectNum<1) {
            ...
            // 商品不合格，天假商品不合格数量
            errorOrderNumLine1+=1;
            return map;
        } else {
            // 检测结果页面的提示信息
            if(results.get(0).getClassName()!="合格"){
                errorOrderNumLine1 += 1;
            }
            map.put("Class", results.get(0).getClassName());
            map.put("Confidence", results.get(0).getConfidence());
        }
